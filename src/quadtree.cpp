...
void Quadtree::build() { // NOLINT(misc-no-recursion)
    if (should_split()) {
        ...
        std::future<void> ne_h = std::async(std::launch::async | std::launch::deferred, &Quadtree::build, m_ne);
        std::future<void> se_h = std::async(std::launch::async | std::launch::deferred, &Quadtree::build, m_se);
        std::future<void> sw_h = std::async(std::launch::async | std::launch::deferred, &Quadtree::build, m_sw);
